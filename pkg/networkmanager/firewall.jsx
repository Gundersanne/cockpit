...
class ActivateZoneModal extends React.Component {
    ...
    onInterfaceChange(event) {
        ...
        this.setState(state => {
            let interfaces = int === "all" ? [] : state.interfaces.filter(i => i !== "all");
            if (state.interfaces.indexOf(int) === -1)
                return { interfaces: interfaces.concat(int) };
            return { interfaces: interfaces.filter(i => i !== int) };
        });
    }
    ...
    save() {
        let interfaces = this.state.interfaces.concat().filter(i => i !== "all");
        let sources = this.state.ipRange === "ip-range" ? this.state.ipRangeValue.split(",").map(ip => ip.trim()) : [];
        firewall.activateZone(this.state.zone, interfaces, sources)
                .then(() => this.props.close())
                .catch(error => {
                    this.setState({
    ...
    render() {
        ...
        let virtualDevices = firewall.availableInterfaces.filter(i => i.capabilities >= 7 && i.device !== "lo").sort((a, b) => a.device.localeCompare(b.device));
        let physicalDevices = firewall.availableInterfaces.filter(i => (i.capabilities < 5 || i.capabilities > 7) && i.device !== "lo").sort((a, b) => a.device.localeCompare(b.device));
        return (
            <Modal id="add-zone-dialog" show onHide={this.props.close}>
                    ...
                    <form className="ct-form-layout">
                        ...
                        <div role="group" className="add-zone-zones">
                            ...
                            <fieldset className="add-zone-zones-custom">
                                <legend>{ _("Custom zones") }</legend>
                                { zones.filter(z => firewall.predefinedZones.indexOf(z) === -1).map(z =>
                                    <label className="radio" key={z}><input type="radio" name="zone" value={z} onChange={e => this.onChange("zone", e.target.value)} />
                                        { firewall.zones[z].name || firewall.zones[z].id }
                                    </label>
                                )}
                        ...
                        <fieldset className="add-zone-interfaces">
                            { physicalDevices.map(i =>
                                <label className="radio" key={i.device}>
                                    <input type="checkbox" value={i.device} onChange={this.onInterfaceChange} checked={this.state.interfaces.indexOf(i.device) !== -1} />
                                    { i.device }
                                </label>) }
                ...
                <Modal.Footer>
                    ...
                    <Button bsStyle="primary" onClick={this.save} disabled={this.state.zone === null || this.state.interfaces.length === 0 ||
                                                                            (this.state.interfaces.indexOf("all") !== -1 && this.state.ipRange === "ip-entire-subnet") ||
